# Consulta webhook PIX cadastrado no Sicoob (GET /webhook/{chave}) via mTLS.
#
# Uso:
#   .\scripts\get-sicoob-webhook.ps1

param(
  [string]$ClientId = '79dda30f-d70f-4ff7-bd74-79509ea137d2',
  [string]$PixKey = '93136803191',
  [string]$Scope = 'webhook.read cob.read pix.read'
)
...
try {
  ...
  foreach ($base in $apiBases) {
    ...
    Write-Host "GET $url"
    $r = Invoke-Mtls -Bundle $bundle -Method 'GET' -Url $url -Headers @{
  ...
  if (-not $found) {
    Write-Host 'Nenhum webhook encontrado (ou API base diferente).'
    exit 1
  }
}
finally {
